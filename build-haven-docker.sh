...
echo "Compiling patched monero code through Docker..."
mkdir monero/build/release/bin -p
docker build --build-arg BA1="$1" --build-arg BA2="$2" --build-arg BA3="$3" -t haven-$HAVENVER -f monero/Dockerfile monero
docker run -v $(pwd)/monero/build/release/bin:/src/build/release/bin_local haven-$HAVENVER bash -c  "cp /src/build/release/bin/* /src/build/release/bin_local/"
